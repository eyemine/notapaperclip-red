            ...
            <div key={i} style={{
              ...
              <span className="mono" style={{ fontSize: '0.72rem', color: 'var(--red)' }}>
                {ev.agentId === '?' ? <span style={{ color: 'var(--muted)' }}>?</span> : (
                  <a href={`/osint?mode=token&agent=${encodeURIComponent(ev.agentId)}&chain=${ev.chainId === 100 ? 'gnosis' : ev.chainId === 8453 ? 'base' : ev.chainId === 84532 ? 'baseSepolia' : ev.chainId === 1 ? 'ethereum' : 'gnosis'}`} style={{ color: 'var(--red)', textDecoration: 'none' }}>#{ev.agentId}</a>
                )}
              </span>
